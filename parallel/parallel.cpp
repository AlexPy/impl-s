...
void boruvkaMST(Graph* graph, std::set<int> *roots)
{
	...
	#pragma offload_transfer target(mic:MIC_COUNT) in(roots, subsets, cheapest, numTrees) nocopy(MSTweight : length(l) ALLOC) //transer data 
	...
	while (numTrees > 1)
	{
		...
		omt_set_num_threads(240);
		#pragma offload target(mic:MIC_COUNT)
		...
		//A NEW ONE IMPLEMENTATION
		omp_set_num_threads(roots->count());		
	...
	#pragma offload target(mic:MIC_COUNT) out(MSTweight:length(l) REUSE)		
	printf("Weight of MST is %d\n", MSTweight);
	#pragma offload_transfer target(mic:MIC_COUNT) nocopy(MSTweight: length(l) FREE)
